...
- (void)viewDidLoad {
    ...
    // 在这里写下需要统计的代码的目录
    NSString *path = @"/Users/changcai/Desktop/iOS-chasen";

    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSInteger strNum =  codeLineCount(path);
        NSLog(@"当前线程：%@",[NSThread currentThread]);
        dispatch_async(dispatch_get_main_queue(), ^{
            // UI更新代码
            label.text = [NSString stringWithFormat:@"在这个文件中代码行数是：%lu",(unsigned long)strNum];
        });
    });
    ...
    NSBlockOperation  *op = [NSBlockOperation blockOperationWithBlock:^{
        NSLog(@"当前线程：%@",[NSThread currentThread]);
        NSLog(@"在这个文件中代码行数是：%lu",(unsigned long)codeLineCount2(path,path));
    }];
...
//            NSString *newPath = [path stringByReplacingCharactersInRange:rect withString:@""];
            if([pathE isEqualToString:h] || [pathE isEqualToString:m] || [pathE isEqualToString:c] || [pathE isEqualToString:mm]|| [pathE isEqualToString:swift]){
                num = test2(path);
//                NSLog(@"%@  ---  %lu行",newPath,(unsigned long)num);
