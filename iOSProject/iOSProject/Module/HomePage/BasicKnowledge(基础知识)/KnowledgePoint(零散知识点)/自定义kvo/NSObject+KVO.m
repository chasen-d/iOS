...
#pragma mark - CSObservationInfo
@interface CSObservationInfo : NSObject

@property (nonatomic, weak) NSObject *observer;
@property (nonatomic, copy) NSString *key;
@property (nonatomic, copy) CSObservingBlock block;

@end

@implementation CSObservationInfo

- (instancetype)initWithObserver:(NSObject *)observer Key:(NSString *)key block:(CSObservingBlock)block
{
    self = [super init];
    if (self) {
        _observer = observer;
...
static void kvo_setter(id self, SEL _cmd, id newValue)
{
    ...
    for (CSObservationInfo *each in observers) {
...
- (void) CS_addObserver:(NSObject *)observer
    ...
    if (![clazzName hasPrefix:kCSKVOClassPrefix]) {
    ...
    // Add this observation info to saved observation objects
    CSObservationInfo *info = [[CSObservationInfo alloc] initWithObserver:observer Key:key block:block];
    NSMutableArray *observers = objc_getAssociatedObject(self, (__bridge const void *)(kCSKVOAssociatedObservers));
    if (!observers) {
        observers = [NSMutableArray array];
        objc_setAssociatedObject(self, (__bridge const void *)(kCSKVOAssociatedObservers), observers, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
{
    NSString *kvoClazzName = [kCSKVOClassPrefix stringByAppendingString:originalClazzName];
